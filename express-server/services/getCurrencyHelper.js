...
async function getCurrencyHelper(currency) {
  ...
  if (myCache.has(currency)) {
    ...
    for (let i = 0; i < supportedCurrencies.length; i++) {
      ...
      try {
        ...
        throw new Error("CryptoAppError: " + e.message + ": " + e.response.statusText);
      }

      apiResponse = apiResponse.data;

      myCache.set(supportedCurrencies[i], apiResponse, 1);
      continue; //FIX: for loop stopping after first iteration
    }
...
module.exports = getCurrencyHelper;
